...
class PPO:
    def __init__(self, state_dim, action_dim, conf_ppo, use_gpu=False):
        ...
        self.set_device(use_gpu)
        self.policy = ActorCritic(state_dim, action_dim, action_std, NN_conf=conf_ppo['nn_type'], use_gpu=use_gpu).to(
            self.device)
        self.policy_old = ActorCritic(state_dim, action_dim, action_std, NN_conf=conf_ppo['nn_type'],
